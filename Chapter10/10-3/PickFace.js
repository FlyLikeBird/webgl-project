...
//   '  vec3 color = (face == u_PickedFace) ? vec3(1.0, 0.0, 0.0) : vec3(0.0, 0.8, 0.0);\n' +
...
'void main() {\n' +
  '  gl_Position = u_ProjectMatrix * u_ViewMatrix * u_ModelMatrix * a_Position;\n' +
  '  int face = int(a_Face);\n' + // Convert to int
  '  vec3 color = (face == u_PickedFace) ? vec3(1.0, 0.0, 0.0) : vec3(0.0, 0.5, 0.0);\n' +
  '  if( u_PickedFace == 0 ) {\n' + // In case of 0, insert the face number into alpha
...
function main(){
    ...
    var a_Face = gl.getAttribLocation(gl.program, 'a_Face');
    ...
    function initVertexBuffer(){
        ...
        var faces = new Uint8Array([
            ...
            5, 5, 5, 5,  //v2-v3-v4-v7 底表面
            6, 6, 6, 6,  //v6-v5-v4-v7 后表面
        ]);
        var n = 6;
    ...
    canvas.onmousedown = function(ev){
        ...
        if ( rect.left <= x && x < rect.right && rect.top <= y && y < rect.bottom ) {
            let innerX = x - rect.left, innerY = rect.bottom - y;
            console.log(innerX, innerY);
            var face = checkFace(innerX, innerY);
